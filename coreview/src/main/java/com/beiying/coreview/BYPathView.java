...
/**
 * Created by beiying on 15/8/19.
 */
public class BYPathView extends View{
    /**
     * Logging tag.
     */
    public static final String LOG_TAG = "BYPathView";
    /**
     ...
     */
    public BYPathView(Context context) {
        this(context, null);
    }
     ...
     */
    public BYPathView(Context context, AttributeSet attrs) {
     ...
     */
    public BYPathView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void getFromAttributes(Context context, AttributeSet attrs) {
        final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.BYPathView);
        try {
            if (a != null) {
                paint.setColor(a.getColor(R.styleable.BYPathView_pathColor, 0xff00ff00));
                paint.setStrokeWidth(a.getFloat(R.styleable.BYPathView_pathWidth, 8.0f));
                svgResourceId = a.getResourceId(R.styleable.BYPathView_svg, 0);
            }
        } finally {
            if (a != null) {
                a.recycle();
    ...
    private void updatePathsPhaseLocked() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            svgPath.mPath.reset();
            svgPath.mMeasure.getSegment(0.0f, svgPath.mLength * progress, svgPath.mPath, true);
            // Required only for Android 4.4 and earlier
            svgPath.mPath.rLineTo(0.0f, 0.0f);
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        synchronized (mSvgLock) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                final Path path = svgPath.mPath;
                final Paint paint1 = naturalColors ? svgPath.mPaint : paint;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (SvgUtils.SvgPath path : paths) {
            desiredWidth += path.mBounds.left + path.mBounds.width() + strokeWidth;
            desiredHeight += path.mBounds.top + path.mBounds.height() + strokeWidth;
        }
        int widthSize = MeasureSpec.getSize(widthMeasureSpec);
        int heightSize = MeasureSpec.getSize(heightMeasureSpec);
    ...
    public static class AnimatorBuilder {
         ...
         */
        public AnimatorBuilder(final BYPathView pathView) {
