...
/**
 * Created by beiying on 2015/12/28.
 * 原理：基于属性动画，在属性变化的过程中通过Android图像合成的技术（需要mask）改变view的显示
 */
public class BYShimmerLayout extends FrameLayout{
    private static final String TAG = "BYShimmerLayout";
    ...
    public BYShimmerLayout(Context context) {
        this(context, null, 0);
    }
    public BYShimmerLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public BYShimmerLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void startShimmerAnimator() {
    ...
    public Animator getShimmerAnimator() {
        if (mShimmerAnimator != null) {
        ...
        mShimmerAnimator = ValueAnimator.ofFloat(0.0f, 1.0f + (float) mRepeatDelay / mDuration);
        mShimmerAnimator.setDuration(mDuration + mRepeatDelay);
        mShimmerAnimator.setRepeatCount(mRepeatCount);
        mShimmerAnimator.setRepeatMode(mRepeatMode);
