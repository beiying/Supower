...
/**
 * 输入文字时，文字显示带动画的文本框
 * 原理：在seText或onTextChanged时通过属性动画改变要显示文字的位置
 * Created by beiying on 2016/4/7.
 */
public class BYAnimatedEditText extends AppCompatEditText {
    ...
    public BYAnimatedEditText(Context context) {
        super(context);
        init(context, null);
    }

    public BYAnimatedEditText(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public BYAnimatedEditText(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    private void init(Context context, AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.BYAnimatedEditText, 0, 0);
        try {
            TypedValue outValue = new TypedValue();
            ta.getValue(R.styleable.BYAnimatedEditText_animationType, outValue);
            if (outValue.data == 0) {
            ...
            mMask = ta.getString(R.styleable.BYAnimatedEditText_textMask);
            mAnimatedClear = ta.getBoolean(R.styleable.BYAnimatedEditText_animateTextClear, mAnimatedClear);
            //Only allow animate cursor feature on API 16+
            mAnimateCursor = ta.getBoolean(R.styleable.BYAnimatedEditText_animateCursor, mAnimateCursor);
    ...
    public void setText(CharSequence text, final BufferType type) {
        ...
        if (mAnimated && mAnimatedClear && mPaint != null && TextUtils.isEmpty(text)) {
            BYAnimationEndListener endListener = new BYAnimationEndListener() {
                @Override
                public void onAnimationEnd(Animator animation) {
                    BYAnimatedEditText.super.setText(null, type);
                }
            };
    ...
    private void animateInFromBottom(boolean reverse, BYAnimationEndListener listener) {
        ...
        animUp.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                BYAnimatedEditText.this.invalidate();
            }
        });
    ...
    private void animateInFromRight(boolean reverse, BYAnimationEndListener listener) {
    ...
    private void animateInFromMiddle(boolean reverse, BYAnimationEndListener listener) {
    ...
    private void animatePopIn(final boolean reverse, BYAnimationEndListener listener) {
    ...
    private ValueAnimator animateMoveCursor(boolean reverse) {
        ...
        if (isCursorVisible()) {
            va.addListener(new BYAnimationEndListener() {
                @Override
    ...
    public abstract class BYAnimationEndListener implements Animator.AnimatorListener {
